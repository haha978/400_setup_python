...
def generate_pulses(inst):
    ...
    p1 = defPulse(amp = 1, mod = 0, length = 100e-6, phase = 90, spacing = 50e-6)
    p2 = defPulse(amp = 1, mod = 0, length = 100e-6, phase = 90, spacing = 50e-6)
    ...
    inst.makeBlocks([b1, b2], 1, [500, 500])
    inst.set_interpolation(ch = 1, interp_factor = 8)
    inst.set_NCO(cfr = cfr, phase = 90)
    numFrames = (b1['reps'][0] + b1['reps'][1])*500 + (b2['reps'][0] + b2['reps'][1])*500
